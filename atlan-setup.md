...
## Run the Atlan Sync Workflow

Use the Atlan workflow when your repository contains Atlan asset exports.

```yaml
name: Agents Schema Atlan
...
jobs:
  agents-schema-atlan:
    uses: fivetran/agents_schema/.github/workflows/agents-schema-atlan.yml@v0.0.6
    with:
      metadata-path: metadata/atlan-assets.json
    secrets: inherit
```

`metadata-path` is required; set it to a file or directory containing JSON or YAML exports from Atlan asset/search APIs.
The example uses `metadata/atlan-assets.json`; change it to match your repository.
